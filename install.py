...
release_url = (
    "https://api.github.com/repos/MacHu-GWU/alfred-cdk-ref/releases?per_page=1"
)
...
for (
    ...
    (
        ...
        path_cdk_python_index,
    )
]:
    path_data.write_text(Path(dir_here, path_data.name).read_text())
    path_setting.write_text(Path(dir_here, path_setting.name).read_text())
    shutil.rmtree(str(path_whoosh_index), ignore_errors=True)
